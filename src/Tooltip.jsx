...
function Tooltip({ show, onClose, parentRef, tableNum }) {
    ...
    const fetchTableLists = async () => {
        try {
            const response = await fetch(
                `${import.meta.env.VITE_API_BASE_URL}/get_merge_seat_lists?tableNum=${tableNum}`
            );
        ...
        <div className="tooltip-modal-overlay" onClick={() => { onClose() }} > {/*onClick={onClose}*/}
            ...
            >
                ...
                <div className="status-card">
                        ...
                        <div className="status-content">
                            <ul className="status-list">
                                {successTableList?.map((tableNum) => (
                                    <li key={tableNum} className="status-item status-item-success" onClick={() => handleRemoveTable(tableNum)}>테이블 {tableNum}</li>
                                ))}
                            </ul>
                            <form className="status-actions" onSubmit={handleSuccesSubmit}>
                                <input
                                ...
                                />
                                <button type="submit" className="status-add-btn">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="17" height="16" viewBox="0 0 17 16" fill="none">
                                        <path d="M8.49967 3.33398V12.6673M3.83301 8.00065H13.1663" stroke="#FBFBFB" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round" />
                                    </svg>
                                </button>
                            </form>
                        </div>
                    ...
                    {/* 거절 Section */}
                    <div className="status-container">
                        <div className="status-header status-header-fail">
                            거절 <span className="icon"><svg xmlns="http://www.w3.org/2000/svg" width="15" height="15" viewBox="0 0 15 15" fill="none">
                                <g clipPath="url(#clip0_4210_5361)">
                                    <path d="M9.375 5.625L5.625 9.375M5.625 5.625L9.375 9.375M13.75 7.5C13.75 10.9518 10.9518 13.75 7.5 13.75C4.04822 13.75 1.25 10.9518 1.25 7.5C1.25 4.04822 4.04822 1.25 7.5 1.25C10.9518 1.25 13.75 4.04822 13.75 7.5Z" stroke="#A40732" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
                                </g>
                                <defs>
                        ...
                        <div className="status-content">
                            <ul className="status-list">
                                {failTableList?.map((tableNum) => (
                                    <li className="status-item status-item-fail">테이블 {tableNum}</li>
                                ))}
                            </ul>
                            <form className="status-actions" onSubmit={handleFailSubmit}>
                                <input
                                    className="status-table-num-input"
                                    type="text"
